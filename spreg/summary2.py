...
Adapted from statsmodels.iolib.summary2.summary_col
"""
...
def summary_params(results, yname=None, xname=None, alpha=.05, use_t=True,
    ...
    if isinstance(results, tuple):
        ...
        tvalues = pd.Series(np.asarray(results.z_stat)[:,0],
                            index=name_x)
        pvalues = pd.Series(np.asarray(results.z_stat)[:,1],
                            index=name_x)
...
def _col_info(result, info_dict=None):
    ...
    for i in info_dict:
        ...
        try:
            ...
            out.append('')
        index.append(i)
    out = pd.DataFrame({str(result.name_y): out}, index=index)
    return out
